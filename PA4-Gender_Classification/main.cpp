...
int main()
{
    ...
    readInFaces(maleTrainingFaces, femaleTrainingFaces, "BayesData/16_20/new-trPCA_03.txt", "BayesData/16_20/TtrPCA_03.txt");
    ...
    readInFaces(maleTestFaces, femaleTestFaces, "BayesData/16_20/testPCA_03.txt", "BayesData/16_20/TtestPCA_03.txt");
    ...
    for(unsigned int i = 0; i < femaleTestFaces.size() - 8; i++)
    {
        cout << i << endl;
        if(BayesClassifier::classifierCaseThree(maleTestFaces[i], maleMean, femaleMean, maleCovar, femaleCovar) == 2)
        {
            correct++;
        }
    }
    ...
    cout << "\tTotal tests: \t" << (maleTestFaces.size() + femaleTestFaces.size() -8) << endl;
    cout << "\tCorrect:     \t" << correct << endl;
    cout << "\tPercentage:  \t" << ((float)correct / (float)(maleTestFaces.size() + femaleTestFaces.size() - 8)) << endl;
